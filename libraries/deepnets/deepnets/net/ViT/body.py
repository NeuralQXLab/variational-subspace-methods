    ...
    def setup(self):
        ...
        if self.transl_invariant:
            ...
            if self.two_dimensional:
                # Make a 2D translationally invariant (h, L_eff, L_eff) J matrix, by appropriately
                # repeating the parameters of the original (h, L_eff) J matrix.
                # This assumes that the patches, as defined in Embed.extract_patches are indexed as: [[0,1,...,sqrt(L_eff)-1],
                #                                                                                      [sqrt(L_eff),sqrt(L_eff)+1,...,2*sqrt(L_eff)-1],
                #                                                                                      ...
                #                                                                                      [(sqrt(L_eff)-1)*sqrt(L_eff),(sqrt(L_eff)-1)*sqrt(L_eff)+1,...,L_eff-1]]
                # or the transpose of this, from the real space configurations on the lattice
                #
                sq_L_eff = int(self.L_eff**0.5)
                assert sq_L_eff * sq_L_eff == self.L_eff
                # Make the tensor J[...,i,j,...] where i,j specifies how many times we have shifted the parameter in x and y directions
                self.J = roll2d(
                    self.J, jnp.arange(sq_L_eff), jnp.arange(sq_L_eff)
                )  # [h, sqrt(L_eff), sqrt(L_eff), L_eff]
                # If patching has correct structure, the flattened J will be translationally invariant
